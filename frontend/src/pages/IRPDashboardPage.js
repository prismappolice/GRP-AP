...
import { ArrowUpRight, Building2, Download, FileText, RefreshCw, Upload, Image as ImageIcon, Eye, LogIn, Clock, ThumbsUp, ThumbsDown, AlertCircle, CheckCircle2 } from 'lucide-react';
...
export const IRPDashboardPage = () => {
  ...
  const filteredComplaints = useMemo(() => complaints.filter((row) => {
    ...
    return true;
  }), [complaints, dateFrom, dateTo, crimeTypeFilter, stationFilter, statusFilter]);
  ...
  if (loading) {
    return (
      <div className="min-h-screen pt-8 px-4 bg-[#F8FAFC]">
        <div className="max-w-7xl mx-auto py-12 text-center text-[#475569]">Loading IRP dashboard...</div>
      </div>
    );
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <Card className="mb-6 p-4 border border-[#60A5FA] bg-white">
          <div className="flex flex-wrap items-end gap-3">
            ...
            <button type="button" onClick={() => { setDateFrom(''); setDateTo(''); setCrimeTypeFilter(''); setStationFilter(''); setStatusFilter(''); }} className="inline-flex items-center justify-center gap-1.5 px-4 py-2 rounded-lg bg-[#2563EB] text-white text-sm font-semibold hover:bg-[#1D4ED8] transition-colors border border-[#2563EB]">
