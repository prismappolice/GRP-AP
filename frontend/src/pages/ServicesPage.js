...
import { FileText, HelpCircle, MapPin, Smartphone, Globe2Icon } from 'lucide-react';

export const ServicesPage = () => {
  const services = [
    { icon: FileText, title: 'File e-Complaint', description: 'Register e-complaints online and track status', link: '/complaint', color: 'bg-[#2563EB]' },
    { icon: MapPin, title: 'Station Locator', description: 'Find nearest GRP stations', link: '/stations', color: 'bg-[#16A34A]' }, 
    { icon: null, image: '/shakthi-logo.png', title: 'Women Safety', description: 'SOS and safety helplines', link: '/women-safety', color: 'bg-[#DC2626]' },
            ...
            return (
              <Link key={idx} to={service.link}>
                <Card className="p-8 border border-[#60A5FA] bg-white hover:-translate-y-1 hover:shadow-lg transition-all duration-200 h-full">
                  <div className={`${service.color} w-14 h-14 rounded-md flex items-center justify-center mb-4 overflow-hidden`}>
                    {service.image
