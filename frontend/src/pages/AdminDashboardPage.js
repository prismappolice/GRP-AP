    ...
    <div className="min-h-screen pt-8 bg-[#F8FAFC] pb-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {/* Removed alert message for pending complaints above cards as per user request */}
          {adminServices.map((service, idx) => {
            const Icon = service.icon;
            const isComplaints = service.title === 'Complaints';
            const hasUnassigned = isComplaints && counts.pendingComplaints > 0;
            const linkTo = hasUnassigned ? '/admin-complaints?unassigned=1' : service.link;
            return (
              <Link key={idx} to={linkTo}>
                <Card className={`relative p-8 border bg-white hover:-translate-y-1 hover:shadow-lg transition-all duration-200 h-full ${hasUnassigned ? 'border-[#FCA5A5]' : 'border-[#60A5FA]'}`}>
                  {/* Pulsing dot for unassigned complaints */}
                  {hasUnassigned && (
