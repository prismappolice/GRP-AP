...
import { Mail, Phone, HelpCircle, Clock, CheckCircle2, XCircle } from 'lucide-react';
            ...
            <Table className="border-collapse">
              <TableHeader className="bg-[#F8FAFC]">
                <TableRow className="hover:bg-[#F8FAFC]">
                  <TableHead className="border border-[#E2E8F0] px-4 py-3 w-16 text-center font-bold text-[#0F172A]">S.No</TableHead>
                  <TableHead className="border border-[#E2E8F0] px-4 py-3 font-bold text-[#0F172A]">Name</TableHead>
                  <TableHead className="border border-[#E2E8F0] px-4 py-3 font-bold text-[#0F172A]">Contact</TableHead>
                  <TableHead className="border border-[#E2E8F0] px-4 py-3 font-bold text-[#0F172A]">Message</TableHead>
                  <TableHead className="border border-[#E2E8F0] px-4 py-3 font-bold text-[#0F172A]">Date</TableHead>
                  <TableHead className="border border-[#E2E8F0] px-4 py-3 font-bold text-[#0F172A]">Status</TableHead>
                  <TableHead className="border border-[#E2E8F0] px-4 py-3 font-bold text-[#0F172A]">Action</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {requests.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={7} className="border border-[#E2E8F0] px-4 py-10 text-center text-[#64748B]">
                  ...
                  requests.map((req, index) => (
                    <TableRow key={req.id} className="hover:bg-[#F8FAFC]">
                      ...
                      <TableCell className="border border-[#E2E8F0] px-4 py-3 font-medium text-[#0F172A] whitespace-nowrap">{req.name || '-'}</TableCell>
                      <TableCell className="border border-[#E2E8F0] px-4 py-3">
                      ...
                      <TableCell className="border border-[#E2E8F0] px-4 py-3 max-w-xs">
                        <p className="text-sm text-[#334155] line-clamp-2" title={req.message}>{req.message || '-'}</p>
                      </TableCell>
                      <TableCell className="border border-[#E2E8F0] px-4 py-3 text-[#334155] whitespace-nowrap text-sm">{formatDate(req.created_at)}</TableCell>
                      <TableCell className="border border-[#E2E8F0] px-4 py-3">
                      ...
                      <TableCell className="border border-[#E2E8F0] px-4 py-3">
                        {req.status === 'pending' ? (
                          ...
                          <button
                            className="px-3 py-1 bg-[#F1F5F9] text-[#475569] text-xs font-medium rounded hover:bg-[#E2E8F0] transition-colors border border-[#E2E8F0] disabled:opacity-50"
                            disabled={updatingId === req.id}
