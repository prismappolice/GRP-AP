  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] pt-4 pb-12 px-4">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Station Alerts notification removed as per user request */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          <Card className="p-5 border border-[#60A5FA]">
            ...
            {statusPieData.length > 0 ? (
              <ResponsiveContainer width="100%" height={260}>
                <PieChart>
                  <Pie data={statusPieData} cx="50%" cy="50%" outerRadius={90} dataKey="value" label={({ name, value }) => `${name}: ${value}`} labelLine={false} onClick={(data) => { const val = data.name === 'Closed' ? 'resolved' : data.name; setStatusFilter(prev => prev === val ? '' : val); }} cursor="pointer">
          ...
          <Card className="p-5 border border-[#60A5FA]">
            ...
            {crimeTypeByDateData.data.length > 0 ? (
              <ResponsiveContainer width="100%" height={260}>
                <BarChart data={crimeTypeByDateData.data} margin={{ top: 5, right: 10, left: -10, bottom: 55 }} onClick={(data) => { if (data?.activePayload?.[0]) { const ct = data.activePayload[0].dataKey.replace(/ /g, '_'); setCrimeTypeFilter(prev => prev === ct ? '' : ct); } }} style={{ cursor: 'pointer' }}>
            ...
            {complaintsByMonthData.length > 0 ? (
              <ResponsiveContainer width="100%" height={240}>
                <BarChart data={complaintsByMonthData} margin={{ top: 5, right: 10, left: -10, bottom: 20 }}>
                  ...
                  <YAxis tick={{ fontSize: 11 }} allowDecimals={false} />
                  <Tooltip />
                  <Bar dataKey="value" name="Complaints" fill="#2563EB" radius={[4, 4, 0, 0]} onClick={(data) => { if (dateFrom === data.name && dateTo === data.name) { setDateFrom(''); setDateTo(''); } else { setDateFrom(data.name); setDateTo(data.name); } }} cursor="pointer" />
                </BarChart>
              </ResponsiveContainer>
            ) : (
              ...
              <BarChart data={ubByMonthData} margin={{ top: 5, right: 10, left: -10, bottom: 20 }}
                onClick={(data) => {
                  if (data && data.activeLabel) {
                    setDateFrom(data.activeLabel);
                    setDateTo(data.activeLabel);
                  }
                }}
