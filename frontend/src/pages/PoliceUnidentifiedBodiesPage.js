  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] p-4 sm:p-6">
            ...
            <div className="overflow-x-auto rounded-lg border border-[#60A5FA]">
              <Table className="border-collapse w-full">
                ...
                <TableBody>
                  {filtered.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={5} className="text-center text-[#94A3B8] py-14 text-sm border border-[#60A5FA]">
                        No records found.
                      </TableCell>
                    </TableRow>
                  ) : (
                    sortedFiltered.map((r, i) => (
                      <TableRow key={i} className="hover:bg-[#F8FAFC] transition-colors">
                        <TableCell className="text-sm text-[#94A3B8] py-3 text-center border border-[#60A5FA] px-3 align-center">{i + 1}</TableCell>
                        <TableCell className="font-medium text-[#0F172A] text-sm py-3 whitespace-nowrap border border-[#60A5FA] px-3 align-center">{r.station || '-'}</TableCell>
                        <TableCell className="text-sm text-[#475569] py-3 whitespace-nowrap border border-[#60A5FA] px-3 align-center">{r.reported_date || '-'}</TableCell>
                        ...
                        <TableCell className="py-3 border border-[#60A5FA] px-3 flex items-center justify-center">
                          {r.mediaUrls?.length > 0 ? (
                            <button
                              ...
                              title={`View all ${r.mediaUrls.length} media item${r.mediaUrls.length !== 1 ? 's' : ''}`}
                            >
                              {isVideo(r.mediaUrls[0]) ? (
                                <div className="w-full h-full bg-[#E2E8F0] flex items-center justify-center">
                                  <span className="text-lg font-bold text-[#64748B]">▶</span>
                                </div>
                              ) : (
                                <img src={normalizeMediaUrl(r.mediaUrls[0])} alt="thumb-0" className="w-full h-full object-cover" />
                              )}
                            </button>
                          ) : (
      ...
      <Dialog open={!!viewGroup} onOpenChange={open => { if (!open) { setViewGroup(null); setMediaIndex(0); } }}>
        <DialogContent className="max-w-2xl w-full bg-white/95 backdrop-blur-sm rounded-2xl border-2 border-[#93C5FD] shadow-2xl fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-[1300]">
          <DialogHeader>
          ...
          {viewGroup && (
            <div className="space-y-3">
              ...
              <div className="relative flex items-center justify-center bg-[#F1F5F9] rounded-lg overflow-hidden" style={{maxHeight: '55vh', width: '100%'}}>
