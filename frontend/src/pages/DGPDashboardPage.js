...
export const DGPDashboardPage = () => {
  ...
  useEffect(() => {
    ...
    if (status) setComplaintStatusFilter(status);
  }, [searchParams]);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          <Card className="p-5 border border-[#60A5FA]">
            ...
            {statusPieData.length === 0 ? <p className="text-xs text-[#94A3B8] text-center py-8">No data</p> : (
              <ResponsiveContainer width="100%" height={260}>
                <PieChart>
                  <Pie data={statusPieData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={90} label={({ name, percent }) => `${name} ${(percent*100).toFixed(0)}%`} onClick={(data) => { const val = data.name === 'Closed' ? 'resolved' : data.name; setComplaintStatusFilter(prev => prev === val ? '' : val); }} cursor="pointer">
          ...
          <Card className="p-5 border border-[#60A5FA]">
            ...
            {crimeTypeByDateData.data.length === 0 ? <p className="text-xs text-[#94A3B8] text-center py-8">No data</p> : (
              <ResponsiveContainer width="100%" height={260}>
                <BarChart data={crimeTypeByDateData.data} margin={{ top: 4, right: 8, bottom: 40, left: 0 }} onClick={(data) => { if (data?.activePayload?.[0]) { const ct = data.activePayload[0].dataKey.replace(/ /g, '_'); setComplaintTypeFilter(prev => prev === ct ? '' : ct); } }} style={{ cursor: 'pointer' }}>
            ...
            {complaintsByMonthData.length === 0 ? <p className="text-xs text-[#94A3B8] text-center py-8">No data</p> : (
              <ResponsiveContainer width="100%" height={240}>
                <BarChart data={complaintsByMonthData} margin={{ top: 4, right: 8, bottom: 20, left: 0 }}>
                  ...
                  <YAxis tick={{ fontSize: 11 }} allowDecimals={false} />
                  <Tooltip />
                  <Bar dataKey="value" name="Complaints" fill="#2563EB" radius={[3,3,0,0]} onClick={(data) => { if (fromDate === data.name && toDate === data.name) { setFromDate(''); setToDate(''); } else { setFromDate(data.name); setToDate(data.name); } }} cursor="pointer" />
                </BarChart>
              </ResponsiveContainer>
            )}
          </Card>
